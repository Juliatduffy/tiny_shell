 ...
 * This stuff has to do with stopping and resuming tasks. bg means resume it in the bg so you can still use terminal
 * fg resumes it in the foreground (so you only see the stdout off that process you can't use shell thats why limit 1)
...
char prompt[] = "tsh> ";    /* command line prompt (DO NOT CHANGE) OMGGGG SO SICK */
...
{
  char c;
  char cmdline[MAXLINE]; /* max line is max line size */
  ...
  while ((c = getopt(argc, argv, "hvp")) != EOF) {
    switch (c) {
    case 'h':             /* when user types in h, print help messages */
      usage();
      break;
    case 'v':             /* emit additional diagnostic info... erm wtf */
      verbose = 1;
      break;
    case 'p':             /* don't print a prompt */
      emit_prompt = 0;  /* handy for automatic testing ... erm wtf part 2*/
      break;
    default:
      usage();
    }
  ...
  /* Initialize the job list (not sure what is mean by job, process?) */
  initjobs(jobs);
  ...
  while (1) {
    ...
    /* Read command line */
    if (emit_prompt) { /* i don't fully get this but it is set with flag p*/
      printf("%s", prompt);
      fflush(stdout); //flush the stdout buffer
    }
    if ((fgets(cmdline, MAXLINE, stdin) == NULL) && ferror(stdin))
      app_error("fgets error");
    if (feof(stdin)) { /* End of file (ctrl-d) erm can a shell read from a file */
      fflush(stdout); //flush the stdout buffer
      exit(0);
    }
  ...
  exit(0); /* control never reaches here wow so like the shell just runs and runs until user exits */
}
